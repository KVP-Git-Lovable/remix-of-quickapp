-- Add p_caller param so the RPC works when invoked via service-role from a trusted edge function.
-- Falls back to auth.uid() for direct callers. System-admin check is preserved.
...
REVOKE ALL ON FUNCTION public.partial_ownership_transfer(uuid,uuid,jsonb,boolean,uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.partial_ownership_transfer(uuid,uuid,jsonb,boolean,uuid) TO authenticated;