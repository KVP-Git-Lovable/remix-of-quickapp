...
BEGIN
  ...
  LOOP
    ...
    IF v_rule.receiver_type = 'self' THEN
      v_receiver_ids := ARRAY[NEW.actor_user_id];

    ELSIF v_rule.receiver_type = 'manager' OR v_rule.receiver_type = 'employee' THEN
      SELECT ARRAY(
    ...
    LOOP
      IF v_receiver_id IS NOT NULL AND (v_rule.receiver_type = 'self' OR v_receiver_id != NEW.actor_user_id) THEN
