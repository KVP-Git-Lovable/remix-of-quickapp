...
BEGIN
  ...
  IF v_retailer_ids IS NOT NULL THEN
    ...
    SELECT count(*) INTO v_count FROM public.retailers WHERE id = ANY(v_retailer_ids) AND owner_id IS DISTINCT FROM p_from;
    IF v_count > 0 THEN
      v_warnings := v_warnings || jsonb_build_object('bucket','retailers','message', v_count || ' selected retailer(s) are not currently owned by source user');
    END IF;
    IF NOT p_dry_run THEN
      UPDATE public.retailers SET owner_id = p_to WHERE id = ANY(v_retailer_ids) AND owner_id = p_from;
      GET DIAGNOSTICS v_count = ROW_COUNT;
    ELSE
      SELECT count(*) INTO v_count FROM public.retailers WHERE id = ANY(v_retailer_ids) AND owner_id = p_from;
    END IF;
  ...
  IF v_beat_ids IS NOT NULL THEN
    ...
    -- Warn about retailers under selected beats not owned by source
    ...
    IF NOT p_dry_run THEN
      UPDATE public.beats SET owner_id = p_to WHERE beat_id = ANY(v_beat_ids) AND owner_id = p_from;
      GET DIAGNOSTICS v_count = ROW_COUNT;
    ELSE
      SELECT count(*) INTO v_count FROM public.beats WHERE beat_id = ANY(v_beat_ids) AND owner_id = p_from;
    END IF;
  ...
  IF v_territory_ids IS NOT NULL THEN
    ...
    IF NOT p_dry_run THEN
      UPDATE public.territories SET assigned_user_id = p_to WHERE id = ANY(v_territory_ids) AND assigned_user_id = p_from;
      GET DIAGNOSTICS v_count = ROW_COUNT;
    ELSE
      SELECT count(*) INTO v_count FROM public.territories WHERE id = ANY(v_territory_ids) AND assigned_user_id = p_from;
    END IF;
  ...
  IF v_distributor_ids IS NOT NULL THEN
    ...
    IF NOT p_dry_run THEN
      UPDATE public.distributors SET owner_id = p_to WHERE id = ANY(v_distributor_ids) AND owner_id = p_from;
      GET DIAGNOSTICS v_count = ROW_COUNT;
    ELSE
      SELECT count(*) INTO v_count FROM public.distributors WHERE id = ANY(v_distributor_ids) AND owner_id = p_from;
    END IF;
  ...
  IF v_van_ids IS NOT NULL THEN
    ...
    IF NOT p_dry_run THEN
      UPDATE public.vans SET assigned_user_id = p_to WHERE id = ANY(v_van_ids) AND assigned_user_id = p_from;
      GET DIAGNOSTICS v_count = ROW_COUNT;
    ELSE
      SELECT count(*) INTO v_count FROM public.vans WHERE id = ANY(v_van_ids) AND assigned_user_id = p_from;
    END IF;
  ...
  IF v_direct_report_ids IS NOT NULL THEN
    ...
    IF NOT p_dry_run THEN
      UPDATE public.employees SET manager_id = p_to WHERE user_id = ANY(v_direct_report_ids) AND manager_id = p_from;
      GET DIAGNOSTICS v_count = ROW_COUNT;
    ELSE
      SELECT count(*) INTO v_count FROM public.employees WHERE user_id = ANY(v_direct_report_ids) AND manager_id = p_from;
    END IF;
    v_counts := v_counts || jsonb_build_object('direct_reports', v_count);
  END IF;

  -- Compute affected retailer set: explicitly selected + retailers under selected beats (owned by source)
  ...
  -- Optional: actually move pending payments / open ledger to new owner
  IF v_include_payments AND v_affected_retailers IS NOT NULL AND array_length(v_affected_retailers,1) > 0 THEN
    -- credit_ledger: best-effort; only update if a user_id-style column exists.
    -- We avoid touching unknown columns; record counts as informational.
    IF NOT p_dry_run THEN
      -- distributor_payments has no user_id column verified; skip writes there to avoid silent corruption.
      -- Record what would have moved.
      NULL;
    END IF;
