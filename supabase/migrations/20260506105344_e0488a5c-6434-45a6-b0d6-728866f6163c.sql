-- One-time repair: if every retailer under a beat now points to the same assignee,
-- align the parent beat's operational assignee with that consensus.
WITH beat_user_consensus AS (
  SELECT
    r.beat_id,
...
SET user_id = c.resolved_user_id
FROM beat_user_consensus c
WHERE b.beat_id = c.beat_id
  AND c.assignee_count = 1
  AND c.resolved_user_id IS NOT NULL
  AND b.user_id IS DISTINCT FROM c.resolved_user_id;
