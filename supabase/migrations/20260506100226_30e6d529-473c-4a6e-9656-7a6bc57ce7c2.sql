...
-- Backfill missing product rows for existing event orders
DO $$
DECLARE
  ord RECORD;
  v_event_id uuid;
...
BEGIN
  ...
  LOOP
    ...
    LOOP
      ...
      IF EXISTS (SELECT 1 FROM public.event_stock_audit WHERE order_id = ord.id AND product_id = v_pid) THEN
        CONTINUE;
      END IF;
      ...
      INSERT INTO public.event_stock_audit(
        ...
        v_row.id, v_event_id, v_day_id, v_pid,
        ord.id, ord.visit_id, ord.user_id,
        v_item.quantity, v_row.sold_qty, v_row.sold_qty + v_item.quantity, 'backfill_missing_product'
      );
    END LOOP;
  END LOOP;
END $$;