...
-- 1. Schema additions
-- ============================================================
ALTER TABLE public.beats     ADD COLUMN IF NOT EXISTS user_id    uuid;
ALTER TABLE public.retailers ADD COLUMN IF NOT EXISTS created_by uuid;

-- Backfill: operational assignee on beats = current owner if any, else creator
UPDATE public.beats
   SET user_id = COALESCE(owner_id, created_by)
 WHERE user_id IS NULL;

-- Backfill: historical creator on retailers = current operational user as best proxy
...
-- Allow operational assignee to update their beats (visibility already permissive)
DROP POLICY IF EXISTS "Assignees can update their beats" ON public.beats;
CREATE POLICY "Assignees can update their beats"
  ON public.beats FOR UPDATE
  USING (user_id = auth.uid())
  WITH CHECK (user_id = auth.uid());

-- ============================================================
-- 2. Replace partial_ownership_transfer with the 3-column model
-- ============================================================
DROP FUNCTION IF EXISTS public.partial_ownership_transfer(uuid, uuid, jsonb, boolean);
DROP FUNCTION IF EXISTS public.partial_ownership_transfer(uuid, uuid, jsonb, boolean, uuid);
...
BEGIN
  ...
  -- ============ RETAILERS ============
  -- 3-column model:
  --   created_by : never touched (immutable historical creator)
  --   user_id    : always moves to p_to (operational assignee)
  --   owner_id   : moves only when transfer_ownership = true
  ...
  -- ============ BEATS ============
  -- Same 3-column model on beats. Cascaded retailers under selected beats also follow the rule.
  ...
  -- ============ Pending-payments preview only — never mutate financial tables ============
  ...
  IF v_include_payments THEN
    v_warnings := v_warnings || jsonb_build_object(
      ...
      'message','Historical financial ownership is preserved. The new user sees pending dues for collection because retailer assignment moved to them.'
    );
  END IF;
