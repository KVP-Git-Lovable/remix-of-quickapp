      ...
      SELECT
        ...
        COALESCE(
          ...
          CASE WHEN COALESCE(item->>'id','') LIKE '%_variant_%'
            THEN public._safe_uuid(split_part(item->>'id', '_variant_', 1)) END,
          (SELECT pv.product_id FROM public.product_variants pv WHERE pv.id = public._safe_uuid(item->>'variant_id')),
          (SELECT pv.product_id FROM public.product_variants pv WHERE pv.id = public._safe_uuid(item->>'id')),
          (SELECT p.id FROM public.products p WHERE p.id = public._safe_uuid(item->>'id'))
        ),
        COALESCE(
          public._safe_uuid(item->>'variant_id'),
          CASE WHEN COALESCE(item->>'id','') LIKE '%_variant_%'
            THEN public._safe_uuid(split_part(item->>'id', '_variant_', 2)) END,
          (SELECT pv.id FROM public.product_variants pv WHERE pv.id = public._safe_uuid(item->>'product_id')),
          (SELECT pv.id FROM public.product_variants pv WHERE pv.id = public._safe_uuid(item->>'id'))
        ),
