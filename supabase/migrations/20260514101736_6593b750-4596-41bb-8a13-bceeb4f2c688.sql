...
BEGIN
  ...
  IF v_existing_order_id IS NOT NULL THEN
    ...
    IF v_items_count = 0 AND jsonb_array_length(p_items) > 0 THEN
      ...
      SELECT
        v_existing_order_id,
        -- product_id resolution: explicit, then split composite, then look up variant's parent
        COALESCE(
          ...
          (SELECT pv.product_id FROM public.product_variants pv
            WHERE pv.id = public._safe_uuid(item->>'variant_id')),
          (SELECT pv.product_id FROM public.product_variants pv
            WHERE pv.id = public._safe_uuid(item->>'id'))
        ) AS resolved_product_id,
        -- variant_id resolution: explicit, split composite, product_id-as-variant, or bare id-as-variant
        COALESCE(
          ...
          (SELECT pv.id FROM public.product_variants pv
            WHERE pv.id = public._safe_uuid(item->>'product_id')),
          (SELECT pv.id FROM public.product_variants pv
            WHERE pv.id = public._safe_uuid(item->>'id'))
