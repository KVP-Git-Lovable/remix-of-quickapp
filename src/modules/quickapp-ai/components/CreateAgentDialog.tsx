...
import { Bot, CheckCircle2, Cpu, Database, Loader2, Save, Sparkles, Table2 } from "lucide-react";
import {
...
export function CreateAgentDialog({
  ...
  const submitAgent = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        if (res.status === 403 && body?.code === "admin_only") {
          throw new Error("Only administrators can create AI agents");
        }
        throw new Error(body?.error ?? `Submit failed (${res.status})`);
      }
      setCreated({
        id: String(body?.workflow?.id ?? ""),
        name: String(body?.workflow?.name ?? ""),
        description: String(body?.workflow?.description ?? ""),
        blocks: Array.isArray(body?.blocks) ? body.blocks.map((b: unknown) => String(b)) : [],
      });
      toast({
        title: "Agent created",
      ...
      <DialogContent className="flex h-[100dvh] max-h-[100dvh] w-screen max-w-[100vw] translate-x-0 translate-y-0 flex-col gap-0 rounded-none border-0 p-0 left-0 top-0 sm:rounded-none">
        ...
        <div className="flex-1 overflow-y-auto px-6 py-6">
          <div className="mx-auto w-full max-w-3xl space-y-8">
            ...
            {/* created agent — revealed after a successful Submit */}
            {created && (
              <section className="rounded-xl border-2 border-primary/40 bg-primary/5 p-4">
                ...
                <dl className="space-y-2 text-sm">
                  <div className="flex flex-col gap-0.5 sm:flex-row sm:gap-2">
                    <dt className="w-44 shrink-0 text-muted-foreground">Name</dt>
